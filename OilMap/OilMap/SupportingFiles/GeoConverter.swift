...
//  OilMap
//
//  Created by dave76 on 27/11/2018.
...
fileprivate struct GeographicCoordinateData {
  ...
  init(mapProjectionType: MapProjectionType,
       scaleFactor: Double,
...
class GeoConverter {
  ...
  fileprivate let geoCoorDatas: [MapProjectionType: GeographicCoordinateData]
  fileprivate let espln: Double = 0.0000000001
  
  init() {
    geoCoorDatas = [
      .WGS_84: GeographicCoordinateData(mapProjectionType: .WGS_84,
                                        scaleFactor: 1,
                                        longitudeCenter: 0.0,
                                        latitudeCenter: 0.0,
                                        falseNorthing: 0.0,
                                        ...
                                        major: 6378137.0,
                                        minor: 6356752.3142),
      .KATEC: GeographicCoordinateData(mapProjectionType: .KATEC,
                                       scaleFactor: 0.9999 /* 0.9996 */,
                                      longitudeCenter: 2.23402144255274 /* 2.22529479629277 */,
                                      latitudeCenter: 0.663225115757845,
                                      falseNorthing: 600000.0,
                                      falseEasting: 400000.0,
                                      major: 6377397.155,
                                      minor: 6356078.9633422494),
      .TM: GeographicCoordinateData(mapProjectionType: .TM,
                                    scaleFactor: 1,
                                    ...
                                    major: 6377397.155,
                                    minor: 6356078.9633422494),
      .GRS_80: GeographicCoordinateData(mapProjectionType: .GRS_80,
                                        scaleFactor: 1,
                                        longitudeCenter: 2.21656815003280,
                                        latitudeCenter: 0.663225115757845,
                                        falseNorthing: 500000.0,
                                        falseEasting: 200000.0,
                                        major: 6378137,
                                        minor: 6356752.3142),
      .UTMK: GeographicCoordinateData(mapProjectionType: .UTMK,
                                      scaleFactor: 0.9996,
                                      longitudeCenter: 2.22529479629277,
                                      latitudeCenter: 0.663225115757845,
                                      falseNorthing: 2000000.0,
                                      falseEasting: 1000000.0,
                                      major: 6378137,
                                      minor: 6356752.3141403558)]
  }
  ...
  func getDistianceByWGS84(from: GeographicPoint, to: GeographicPoint) -> Double {
    ...
    let a = pow(sin(latitude / 2), 2) + cos(fromLatitude) * cos(toLatitude) * pow(sin(longitude), 2)
    return 6376.5 * 2 * atan2(sqrt(2), sqrt(1 - a))
  }
  ...
  private func getDistancdeByGRS80(from: GeographicPoint, to: GeographicPoint) -> Double? {
  ...
  private func getDistanceByFromType(fromType: MapProjectionType, from: GeographicPoint, to: GeographicPoint) -> Double? {
    ...
    return getDistianceByWGS84(from: fromPoint!, to: toPoint!)
  }
  ...
  private func geodeticToTm(destination: MapProjectionType, inputPoint: GeographicPoint) -> GeographicPoint? {
    ...
    let deltaLongitude = transformedPoint!.x - geoCoorDatas[destination]!.latitudeCenter
    let sinForInputPointY = sin(transformedPoint!.y)
    let cosFroInputPointY = cos(transformedPoint!.y)
    
    if 0 != geoCoorDatas[destination]!.ind {
      let b = cosFroInputPointY * sin(deltaLongitude)
      if abs(abs(b) - 1) < espln {
        return nil // 무한대 에러
      }
    }
    
    let al = cosFroInputPointY * deltaLongitude
    let als = al * al
    let c = geoCoorDatas[destination]!.esp * cosFroInputPointY * cosFroInputPointY
    let tq = tan(transformedPoint!.y)
    let t = tq * tq
    let con = 1 - geoCoorDatas[destination]!.es * sinForInputPointY * sinForInputPointY
    let n = geoCoorDatas[destination]!.major / sqrt(con)
    let ml = geoCoorDatas[destination]!.major *
      GeographicCoordinateData.mlfn(
        e0: GeographicCoordinateData.e0fn(geoCoorDatas[destination]!.es),
        e1: GeographicCoordinateData.e1fn(geoCoorDatas[destination]!.es),
        e2: GeographicCoordinateData.e2fn(geoCoorDatas[destination]!.es),
        e3: GeographicCoordinateData.e3fn(geoCoorDatas[destination]!.es),
        phi: transformedPoint!.y)
    
    let x = geoCoorDatas[destination]!.scaleFactor * n * al * (1 + als / 6 * (1 - t + c + als / 20 * ( 5 - 18 * t + t * t + 72 * c - 58 * geoCoorDatas[destination]!.esp))) + geoCoorDatas[destination]!.falseEasting
    let y = geoCoorDatas[destination]!.scaleFactor * (ml - geoCoorDatas[destination]!.destinationM + n * tq * (als * (0.5 + als / 24 * (5 - t + 9 * c + 4 * c * c + als / 30 * (61 - 58 * t + t * t + 600 * c - 330 * geoCoorDatas[destination]!.esp))))) + geoCoorDatas[destination]!.falseNorthing
  ...
  private func tmToGeodetic(source: MapProjectionType, inputPoint: GeographicPoint) -> GeographicPoint? {
    let newPoint = GeographicPoint(
      x: inputPoint.x - geoCoorDatas[source]!.falseEasting,
      y: inputPoint.y - geoCoorDatas[source]!.falseNorthing)
    let maxIter = 6
    let con = (geoCoorDatas[source]!.sourceM + newPoint.y / geoCoorDatas[source]!.scaleFactor) / geoCoorDatas[source]!.major
    
    func calculatePhi(value: Double, count: Int) -> Double? {
      let deltaPhi = ((con + GeographicCoordinateData.e1fn(geoCoorDatas[source]!.es) * sin(2 * value) - GeographicCoordinateData.e2fn(geoCoorDatas[source]!.es) * sin(4 * value) + GeographicCoordinateData.e3fn(geoCoorDatas[source]!.es) * sin(6 * value)) / GeographicCoordinateData.e0fn(geoCoorDatas[source]!.es)) - value
    ...
    let pointForInd = ( { () -> GeographicPoint? in
      if 0 != geoCoorDatas[source]!.ind {
        let f = exp(inputPoint.x / geoCoorDatas[source]!.major * geoCoorDatas[source]!.scaleFactor)
        let g = 0.5 * (f - 1 / f)
        let temp = geoCoorDatas[source]!.latitudeCenter + inputPoint.y / (geoCoorDatas[source]!.major * geoCoorDatas[source]!.scaleFactor)
        let h = cos(temp)
        let con = sqrt((1 - h * h) / (1 + g * g))
        let y = temp < 0 ? -(GeographicCoordinateData.asinz(con)) : GeographicCoordinateData.asinz(con)
        let x = 0 == g && 0 == h ? geoCoorDatas[source]!.longitudeCenter : atan(g / h) + geoCoorDatas[source]!.longitudeCenter
    ...
    let pointForPhi = ( { () -> GeographicPoint? in
      if abs(phi!) < Double.pi / 2 {
        ...
        let cosPhi = cos(phi!)
        let tanPhi = tan(phi!)
        let c = geoCoorDatas[source]!.esp * cosPhi * cosPhi
        let cs = c * c
        let t = tanPhi * tanPhi
        let ts = t * t
        let cont = 1 - geoCoorDatas[source]!.es * sinPhi * sinPhi
        let n = geoCoorDatas[source]!.major / sqrt(cont)
        let r = n * (1 - geoCoorDatas[source]!.es) / cont
        let d = newPoint.x / (n * geoCoorDatas[source]!.scaleFactor)
        let ds = d * d
        let x = geoCoorDatas[source]!.longitudeCenter + (d * (1 - ds / 6 * (1 + 2 * t + c - ds / 20 * (5 - 2 * c + 28 * t - 3 * cs + 8 * geoCoorDatas[source]!.esp + 24 * ts))) / cosPhi)
        let partA = n * tanPhi * ds / r
        let partB = 61 + 90 * t + 299 * c + 45 * ts - 252 * geoCoorDatas[source]!.esp - 3 * cs
        let y = phi! - partA * (0.5 - ds / 24 * (5 + 3 * t + 10 * c - 4 * cs - 9 * geoCoorDatas[source]!.esp - ds / 30 * partB))
        
        return GeographicPoint(x: x, y: y)
      } else {
        let x = geoCoorDatas[source]!.longitudeCenter
  ...
  private func geodeticToGeocentric(type: MapProjectionType, inputPoint: GeographicPoint) -> GeographicPoint? {
    ...
    let rn = geoCoorDatas[type]!.major / sqrt(1.0e0 - geoCoorDatas[type]!.es * pow(sinLatitude, 2))
    
    let outputPoint = GeographicPoint(
      x: (rn * height) * cosLatitude * cos(longitude),
      y: (rn * height) * cosLatitude * sin(longitude),
      z: (rn * (1 - geoCoorDatas[type]!.es) + height) * sinLatitude)
  ...
  private func geocentricToGeodetic(type: MapProjectionType, inputPoint: GeographicPoint) -> GeographicPoint {
    ...
    let T1 = inputPoint.z + geoCoorDatas[type]!.minor * geoCoorDatas[type]!.esp * sin3B0
    
    // numerator of Math.cos(phi1)
    let sum = W - geoCoorDatas[type]!.major * geoCoorDatas[type]!.es * cosB0 * cosB0 * cosB0
    ...
    let rn = geoCoorDatas[type]!.major / sqrt(1.0 - geoCoorDatas[type]!.es * sinP1 * sinP1)
    
    // indicates location is in polor region
    ...
    let height = ({ () -> Double in
      if inputPoint.x == 0 && inputPoint.y == 0 && inputPoint.z == 0 {
        return -self.geoCoorDatas[type]!.minor
      }
      if cosP1 >= self.cos67p5 {
        ...
        return inputPoint.z / sinP1 + rn * (self.geoCoorDatas[type]!.es - 1)
      }
    })()
  ...
  private func transform(source: MapProjectionType, destination: MapProjectionType, geoPoint: GeographicPoint) -> GeographicPoint? {
    ...
    if (source == .KATEC || source == .TM) || (destination == .KATEC || destination == .TM) {
      // Conbert to geocentric coordinates.
      if let point = geodeticToGeocentric(type: source, inputPoint: geoPoint) {
        ...
        let newPoint = ({ () -> GeographicPoint? in
          if destination == .KATEC || destination == .TM {
            return geodeticToWGS84(pointAddedWGS84 == nil ? point : pointAddedWGS84!)
          } else {
            return nil
          }
        })()
        ...
        if let point = newPoint {
          return geodeticToGeocentric(type: destination, inputPoint: point)
  ...
  private let cos67p5 = 0.37268343236508977   // cosine of 67.5 degrees
